...
名称：GCC - GNU project c and c++ compiler  
概要：  
...
描述：  
	GCC一般包含预处理，编译，汇编，链接。GCC的全部参数中可允许停止GCC的过程于中间过程。某些参数控制预处理，某些控制编译，汇编，链接。 这些参数并不针对某一特定语言。  
	GCC程序接受参数和文件名作为运算符。许多参数不止包含一个字母，很多多字母的参数。单字幕参数一般不能组合，但-dv是个例外，它表示-d和-v的组合。当使用多个参数时，顺序往往不重要。但重复使用某个参数时，顺序有用。如，多次使用-L,目录按照使用次序搜索。  
	许多参数有常的名称，如-f,-W,eg:-fmove-loop-invariants,-Wformat.他们都有肯定和否定两种形式。eg:-ffoo vs -fno-foo.  
...
OPTIONS
   Option Summary
       ...
       Machine Dependent Options
           ...
           -mcx16 -msahf -mmovbe -mcrc32 -mrecip -mvzeroupper -mmmx  -msse
           -msse2 -msse3 -mssse3 -msse4.1 -msse4.2 -msse4 -mavx -maes -mpclmul
           -mfsgsbase -mrdrnd -mf16c -mfused-madd -msse4a -m3dnow -mpopcnt
           -mabm -mbmi -mtbm -mfma4 -mxop -mlwp -mthreads
   ...
   Options That Control Optimization
       ...
       --param name=value
           ...
           sms-min-sc
               ...
               will generate.  The default value is 2.
   ...
   Hardware Models and Configurations
       ...
       -mcpu=name
           ...
           Linux, and not all architectures are recognised.  If the auto-
       ...
       -march=name
           ...
           on Linux, and not all architectures are recognised.  If the auto-
           detect is unsuccessful the option has no effect.
       ...
       -mv850e2v3
           ...
           preprocessor constants __v850e2v3__ will be defined if this option
           is used.
       ...
       -mv850e
           ...
           -mv850e2v3 are defined then a default target processor will be
       ...
       -mdisable-callt
           ...
           architecture.  The default is -mno-disable-callt which allows the
           CALLT instruction to be used.
   ...
   Options for Code Generation Conventions
       ...
       -fstrict-volatile-bitfields
           ...
           (assuming short is 16 bits on these targets) to force GCC to use 16
           bit accesses instead of, perhaps, a more efficient 32 bit access.

           If this option is disabled, the compiler will use the most
           efficient instruction.  In the previous example, that might be a
           32-bit load instruction, even though that will access bytes that do
           not contain any portion of the bitfield, or memory-mapped registers
