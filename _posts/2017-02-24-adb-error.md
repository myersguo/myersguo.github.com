...
目录是存在的啊,目录不就是`D:\`, 我新建一个新的目录`D:\testdir`，然后执行：   
...
这样就OK了，**WHY?** 难道是adb_dirname不支持disk?   

正当我迟疑不解的时候，我手欠把开发者设置的USB调试关掉后重新打开了。然后，问题竟然没有了，然后再也出现不了了。但知觉告诉我，「一切不容易复现的问题都将复现」，但问题就是木有复现，什么原因呢？    
