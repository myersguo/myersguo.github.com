...
event:事件，事件必须有handler才能执行
handler: 事件的具体执行函数，具体为： 
...
```
$_e: 事件的列表，可以直接添加,eg: $this->onClick=xxx  
...
```
$componet->onClick=function(){}
$handler=$componet->onClick;
```
行为扩展：

```
$componet->onSomething();
```

组件会先检查绑定的行为有没有这个事件；然后再检查是否有匿名函数
