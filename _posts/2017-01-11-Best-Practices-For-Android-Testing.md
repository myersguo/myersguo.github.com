...
UI测试让你确保它的功能满足需求，并实现更高标准的质量保证。   
通常UI测试是通过用户的操作行为来验证功能的正确性。然而，手动的操作是**耗时的(time-consuming)**,**乏味的(tedious)**,**易出错的(error-prone)**。更有效的方式是写UI自动化测试程序来实现。   
...
如果包含跳出APP的行为，比如进入了系统UI或者选了了其他APP数据等，就需要针对跨APP进行测试。    
